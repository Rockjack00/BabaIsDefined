...
function createChoicePoints(thread, point, term, choice_list) {
  var states = choice_list.forEach((c) => {
    return new pl.type.State(
      point.goal.replace(new pl.type.Term("=", [term, new pl.type.Term(c)])),
      point.substitution,
      point
    );
  });
  ...
  var predicates = function () {
    return {
      "reachable/4": function (thread, point, atom) {
        ...
        if (
          pl.type.is_variable(x_obj) ||
          pl.type.is_variable(y_obj) ||
          pl.type.is_variable(game_state)
        ) {
      ...
      "isYou/2": function (thread, point, atom) {
        ...
        console.log(
          `reached isYou/2 with: {you: ${you}, game_state: ${game_state}}`
        );
